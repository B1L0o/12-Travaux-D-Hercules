...
if (!host)
    ...
    void Update()
    {
        if (!host)
        {
            ...
            if (Input.GetMouseButtonDown(0) && timeInBetween_MeleeG < 0)
            {
                var server=GameObject.FindGameObjectWithTag("server");
                server.GetComponent<server>().attackevent();
            ...
            if (Input.GetKeyDown(KeyCode.E) && timeInBetween_Bow < 0)
            {
        ...
        public IEnumerator Melee() // Melee Animation
        {
            if (!isGrounded)
            {
        ...
        IEnumerator PoisonEffectHealthDelay(int damage, float time, float delay)
        {
            ...
            Debug.Log("ENDDDDDD");
            ApplyPoison = false;
            //StopCoroutine(PoisonEffectHealth(damage));
        }
        ...
        private void
            GroundCheck() // Determines whether or not Hercules is grounded ,by using the hitbox under his feets
        {
