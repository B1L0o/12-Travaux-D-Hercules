...
public class Dragon_lift : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        Vector2 target = new Vector2(bossTransform.position.x, dragon.transitionPoint.position.y);
        Vector2 newPos = Vector2.MoveTowards(rb.position, target, speed * Time.fixedDeltaTime);
        rb.MovePosition(newPos);
        if (Math.Abs(bossTransform.position.y - dragon.transitionPoint.position.y) < 0.05d)
        {
